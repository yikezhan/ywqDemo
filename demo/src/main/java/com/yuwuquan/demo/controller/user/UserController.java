...
public class UserController {
    ...
    private static final String VERIFICATION_CODE_KEY_PRE = "phone_code_";//验证码的key前缀
    ...
    public Object  loginByVerificationCode(@RequestParam("phone") String phone, @RequestParam("code") String code){
        Object redisCode = redisUtil.get(VERIFICATION_CODE_KEY_PRE + phone);
    ...
    public Object sendCode(@RequestParam("phone") String phone){
        if(redisUtil.get(VERIFICATION_CODE_KEY_PRE +phone) != null){
            return "验证码已存在，有效期内不能重复发送";
        }
        String verificationCode = "12345";//发送短信验证码操作
        redisUtil.set(VERIFICATION_CODE_KEY_PRE + phone, verificationCode, VERIFICATION_CODE_TIME);
