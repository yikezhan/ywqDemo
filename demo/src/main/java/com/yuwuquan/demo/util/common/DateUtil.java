...
public class DateUtil {
    ...
    public static final String DATE_FORMAT_YYYYMMDDHHMMSS_BYCHS = "yyyy年MM月dd日HH:mm:ss";

    public static final String DATE_FORMAT_YYYYMMDD_SPOT = "yyyy.MM.dd";
    ...
    public static String toYMDSpotString(Date date) {
        if (date != null) {
            return new SimpleDateFormat(DATE_FORMAT_YYYYMMDD_SPOT).format(date);
        } else {
            return null;
        }
    }
    ...
    public static String toYMDStringHM(Date date) {
        if (date != null) {
            return new SimpleDateFormat(DEFAULT_DATE_HHMM).format(date);
        } else {
            return null;
        }
    }
    ...
    public static Date parseDateHM(String dateStr) {
        return parse(dateStr, DEFAULT_DATE_HHMM);
    }
    ...
    public static String get4yMdHmsBYCHS(Date date){return getDateFormate(date, DATE_FORMAT_YYYYMMDDHHMMSS_BYCHS);}
    ...
    /**
     * 判断给定时间是否大于当前时间（日期相等返回TRUE:给定日期+1天）
     */
    ...
    /**获取某天指定时间*/
    public static Date getDateByDate(Date date,Integer hour, Integer minute, Integer second)
    {
        Date firstDate = null;
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
    ...
    public static LocalDate formatLocalDate(String date) {
        ...
        return DateUtil.parseDate(date).toInstant().atZone(zoneId).toLocalDate();
    }
    ...
    public static String localDateToString(LocalDate localDate) {
        DateTimeFormatter fmt = DateTimeFormatter.ofPattern(DATE_FORMAT_YYYYMMDD);
